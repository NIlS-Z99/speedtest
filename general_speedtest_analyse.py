...
NO_SHOW = False
SAVE_FILE = False
DIR_PATH = dirname(abspath(argv[0]))

system('color')
filenames = glob(join(DIR_PATH,"speedtest_logs","*.csv"))
...
for filename in filenames:
    ...
    if dataFrame.empty or len(dataFrame["Timestamp"])<1: continue
    ...
    for down_speed,up_speed in zip(dataFrame["Download"],dataFrame["Upload"]):
        if(ceil(down_speed)<MAX_DOWN*0.9): insufficent_down.append(down_speed)
        if(ceil(up_speed)<MAX_UP*0.9): insufficent_down.append(down_speed)
    ...
    for it in range(int(len(dataFrame['Timestamp'])/30)):
        ...
        plt.savefig(join(DIR_PATH,"plots",str('Speedtest Results '+filename.split('_')[-1].split('-')[0]+'#'+str(it)+".png")))
